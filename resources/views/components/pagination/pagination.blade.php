...
<nav role="navigation" aria-label="Pagination Navigation"
    class="flex items-center md:mx-5 mx-2 xl:mx-10 justify-between">
            ...
            <span class="relative z-0 inline-flex rounded-md shadow-sm">
                ...
                <span>
                    ...
                    <button wire:click="previousPage('{{ $paginator->getPageName() }}')"
                        dusk="previousPage{{ $paginator->getPageName() == 'page' ? '' : '.'}}" rel="prev"
                        class="relative mx-4 inline-flex items-center px-2 py-2 text-sm font-medium text-gray-500 bg-black/10 dark:bg-white  border-none border-gray-300 rounded-l-md leading-5 hover:text-gray-400 focus:z-10 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:bg-gray-100 active:text-gray-500 transition ease-in-out duration-150"
                        aria-label="{{ __('pagination.previous') }}">
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd"
                                d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z"
                                clip-rule="evenodd" />
                        </svg> Previous Reviews
                    </button>
                    @endif
                </span>
                ...
                <span class="">
                    ...
                    <button wire:click="nextPage('{{ $paginator->getPageName() }}')"
                        dusk="nextPage{{ $paginator->getPageName() == 'page' ? '' : '.'}}" rel="next"
                        class="relative mx-4 inline-flex items-center px-2 py-2 -ml-px text-sm font-medium text-gray-500 bg-black/10 dark:bg-white border-none border-gray-300 rounded-r-md leading-5 hover:text-gray-400 focus:z-10 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:bg-gray-100 active:text-gray-500 transition ease-in-out duration-150"
                        aria-label="{{ __('pagination.next') }}">
                        Next Reviews<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd"
